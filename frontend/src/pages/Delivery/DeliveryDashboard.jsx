...
import { fetchAssignedOrders } from "../../services/read/order.service";
...
const DeliveryDashboard = () => {
  ...
  const normalizeStatus = (status) => status?.toString().toLowerCase() || "pending";
  ...
  const updateStats = (ordersList) => {
    ...
    const outForDelivery = ordersList.filter(o => o.status?.toLowerCase() === "out for delivery").length;
    const delivered = ordersList.filter(o => o.status?.toLowerCase() === "delivered").length;
    const pending = ordersList.filter(o => o.status?.toLowerCase() !== "delivered").length;
  ...
  const filterOrders = () => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(order => {
        ...
        const address = order.shippingAddress?.street || order.address || "";

        return (
          (order.orderId || order._id || "").toString().toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const getNextAction = (status) => {
    ...
    switch(normalized) {
      case "packed":
        return { label: "Mark as Shipped", status: "shipped", color: "bg-purple-600" };
      case "shipped":
        return { label: "Mark Out for Delivery", status: "out for delivery", color: "bg-orange-600" };
      case "out for delivery":
        return { label: "Mark Delivered", status: "delivered", color: "bg-green-600" };
      default:
        return null;
    }
  };
                      ...
                      >
                        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                          <div className="flex items-center gap-4">
                            <div className={`flex h-12 w-12 items-center justify-center rounded-full ${getStatusColor(normalizedStatus)} text-white`}>
                              {getStatusIcon(normalizedStatus)}
                            </div>
                            <div>
                              <p className="text-xs text-gray-500">Order ID</p>
                              <p className="font-mono font-semibold text-gray-900">#{order.orderId || order._id || orderKey}</p>
                            </div>
                          </div>
                          ...
                          <div className="flex flex-wrap items-center gap-6">
                            <div>
                              ...
                              <p className="font-medium text-gray-900">{order.shippingAddress?.name || order.customerName || order.userId?.name || order.name || order.email}</p>
                            </div>
                            <div>
                              <p className="text-xs text-gray-500">Status</p>
                              <div className={`inline-flex items-center gap-2 rounded-full ${getStatusColor(normalizedStatus)} px-3 py-1 text-white`}>
                                {getStatusIcon(normalizedStatus)}
                                <span className="text-sm font-semibold">{normalizedStatus}</span>
                              </div>
                            </div>
                        ...
                        <div className="px-6 pb-4 text-sm text-gray-600">
                          <div className="flex flex-wrap gap-4">
                            <div className="rounded-2xl bg-gray-50 px-3 py-2">
                              <span className="font-semibold text-gray-800">Address:</span> {order.shippingAddress?.street || order.address || "N/A"}, {order.shippingAddress?.city || order.city || ""}
                            </div>
                            <div className="rounded-2xl bg-gray-50 px-3 py-2">
                              <span className="font-semibold text-gray-800">Items:</span> {order.items?.reduce((sum, item) => sum + (item.quantity || 0), 0) || "N/A"}
                            </div>
                          </div>
                        </div>
                        ...
                        {nextAction && !isDelivered && (
                          <div className="mt-4 flex flex-wrap gap-3">
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                updateStatus(orderKey, nextAction.status);
                              }}
                              disabled={saving && updatingOrderId === orderKey}
                            ...
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                handleWhatsApp(order.shippingAddress?.phone || order.phone, order.orderId || order._id || orderKey);
                              }}
                            ...
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                handleDirections(`${order.shippingAddress?.street || order.address}, ${order.shippingAddress?.city || order.city}`);
                              }}
                            ...
                            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                              ...
                              <div className="rounded-xl bg-white p-4 shadow-sm">
                                ...
                                <p className="text-sm text-gray-600">
                                  {order.shippingAddress?.street || order.address || ""}, {order.shippingAddress?.city || order.city || ""}
                                  {order.shippingAddress?.pincode || order.pincode ? ` - ${order.shippingAddress?.pincode || order.pincode}` : ""}
                                </p>
                              </div>
                              ...
                              <div className="rounded-xl bg-white p-4 shadow-sm">
                                ...
                                {order.items?.length ? (
                                  ...
                                  <p className="text-sm text-gray-600">{order.products || order.productSummary || "No item details available"}</p>
                                )}
                                <div className="mt-3 pt-3 border-t border-gray-100">
                                  <div className="flex justify-between text-sm">
                                    <span className="text-gray-500">Total Items:</span>
                                    <span className="font-semibold">
                                      {order.items?.reduce((sum, item) => sum + (item.quantity || 0), 0) || "N/A"}
                                    </span>
                                  </div>
                                </div>
                              </div>
