...
const Section1 = () => {
  ...
  // Field validators
  const validators = {
    name: (value) => (value.trim().length >= 3 ? null : "Name must be at least 3 characters"),
    email: (value) => (!value || validateEmail(value) ? null : "Enter a valid email address"),
    phone: (value) => (validatePhone(value) ? null : "Enter a valid 10-digit phone number"),
    address: (value) => (value.trim().length >= 10 ? null : "Address must be at least 10 characters"),
    city: (value) => (value.trim().length >= 2 ? null : "Enter a valid city name"),
    pincode: (value) => (validatePincode(value) ? null : "Enter a valid 6-digit pincode"),
    landmark: () => null,
    state: () => null,
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const resp = await saveOrder(payload);
      clearCart();
      const createdOrderId = resp?.order?.orderId || resp?.order?._id || null;
      navigate("/success", { state: { orderId: createdOrderId, total, paymentMethod } });
    } catch (error) {
      console.error(error);
